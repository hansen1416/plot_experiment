...
def _generate_flood_png3857(muids, flood_data):
    ...
    #     'AUTHORITY[\"EPSG\",\"2193\"]]'
    # transformer_dhi3857 = pyproj.Transformer.from_crs(
    #     crs_dhi, pyproj.CRS.from_epsg(3857))
    ...
    # rectangle3857 = {'x_0': x3857[0], 'x_n': x3857[1],
    #                  'y_0': y3857[0], 'y_n': y3857[1]}
    # figure size needs to be proportionate to the coordinates
    figsize = (round(grid2D['numJ']/10), round(grid2D['numK']/10))
    marker_size = 100
    ...
    plt.scatter(muids_masked[:, 0], muids_masked[:, 1], s=marker_size, c=flooding_masked,
    ...
    img.save(os.path.join('tmp', 'muids_int_{}_{}.png'.format(
        str(figsize), marker_size)), format='PNG')
...
def muidsgrid(muids):
    ...
    crs_dhi = 'PROJCS[\"NZGD2000 / New Zealand Transverse Mercator 2000\",GEOGCS[\"NZGD2000\",DATUM[\"New_Zealand_Geodetic_Datum_2000\",' +\
        ...
        'AUTHORITY[\"EPSG\",\"2193\"]]'
    transformer_dhi3857 = pyproj.Transformer.from_crs(
        pyproj.CRS.from_epsg(3857), crs_dhi)

    muids1 = np.zeros(muids.shape)

    muids1[:, 0], muids1[:, 1] = transformer_dhi3857.transform(
        muids[:, 0], muids[:, 1])
    ...
    muids1 = np.array(muids1, dtype=int)
...
if __name__ == "__main__":
    ...
    _generate_flood_png3857(muids1, data)
